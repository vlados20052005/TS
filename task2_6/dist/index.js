...
const toggleCrossedOut = (taskDiv) => {
...
const saveTasks = () => {
    ...
    if (taskElements) {
        const tasksArray = Array.from(taskElements).map((task) => {
            return {
                text: task.querySelector(".item").textContent || "",
                checked: task.querySelector(".checked").checked,
            };
        });
...
const loadTasks = () => {
    ...
    if (savedTasks) {
        ...
        tasksArray.forEach((task) => {
            ...
            const taskDiv = tasks?.lastElementChild;
            if (taskDiv) {
                taskDiv.querySelector(".checked").checked =
                    task.checked;
                if (task.checked) {
                    taskDiv.classList.add("cross-out");
                    taskDiv.classList.remove("not-crossed-out");
