export const today = (data: any): string => {
  return `<div class="temprature-info">
            <div class="now">${(data.list[0].main.feels_like - 273.15).toFixed(
              2
            )}°C</div>
            <div class="warmest"><div>${
              data.list[0].weather[0].main
            }</div><div>${(data.list[0].main.temp - 273.15).toFixed(
    2
  )}°C</div></div>
          </div>
...
export const anotherDay = (
  dayData: any,
  dayTemps: { minTemp: number; maxTemp: number },
  nightTemps: { minTemp: number; maxTemp: number }
): string => {
...
export const celected = (data: any): string => {
